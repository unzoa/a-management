...
unzoa.log = console.log.bind(console);
unzoa.find    = function (ev) {return document.querySelector(ev); }
unzoa.findAll = function (ev) {return document.querySelectorAll(ev); }
unzoa.toStr   = function (ev) {return JSON.stringify(ev); }
unzoa.toJson  = function (ev) {return JSON.parse(ev); }

// window.onload:dom文档树加载完和所有文件加载完
// ready:dom文档树加载完
// 预先加载的loading:可以在ready写程序，window.load中写停止加载
unzoa.ready = function (callback) {
    ///兼容FF,Google
...
unzoa.ajax = function(options){//options =  {url:'',method:'',data:'',callback:'',async:''}
    //默认参数
...
// 设置cookie
// _time. ms
unzoa._setCookie = function (name, value, _time) {
    //定义一天
    var days = 1;
    var exp = new Date();
    //定义的失效时间，
...
// 获取cookie
unzoa._getCookie = function (cookie_name) {
...
// 获取url参数
unzoa._getUrlParam  = function (name) {
...
// 取到radio的值
unzoa._getRadio = function (radioName){
...
// 获取checkbox的值
unzoa._getCheckBox = function (boxName){
